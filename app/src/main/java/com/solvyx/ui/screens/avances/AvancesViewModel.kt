...
class AvancesViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            ...
            .collect { (bitacora, logros) ->
                ...
                feelingsDataSemana = semanaDays.map { d ->
                    entryMap[d]?.firstOrNull()?.let { moodScale[it.estadoAnimo] } ?: 0f
                }
                consumoSemana = semanaDays.map { d ->
                    if (entryMap[d]?.any { it.consumio } == true) 1f else 0f
                }
    ...
    private fun autoUnlock(logros: List<LogroEntity>, currentRacha: Int) {
        ...
        logros.filter { !it.unlocked }.forEach { logro ->
            ...
            if (currentRacha >= threshold) {
                viewModelScope.launch { repository.desbloquearLogro(logro.id) }
            }
        }
    }

    private fun mapLogro(entity: LogroEntity): UiLogro {
